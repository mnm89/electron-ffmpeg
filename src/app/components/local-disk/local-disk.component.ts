...
import { Tree, ChildrenTree, VIDEO_EXTENTIONS, IMAGE_EXTENTIONS } from '../../helpers';
import { Store } from '@ngrx/store';
import { AppState } from '../../store/state';
import { CURRENT_TREE, ADD_TO_LOCAL_PLAY_LIST } from '../../store/Local/reducer';
...
export class LocalDiskComponent implements OnInit {
  ...
  @ViewChild('playlistselection') playlistselection: ElementRef<HTMLDivElement>
  @ViewChild('inputSelectFolder') inputSelectFolder: ElementRef<HTMLInputElement>
  @ViewChild('inputFilterFolder') inputFilterFolder: ElementRef<HTMLInputElement>
  ...
  onDirChange(event) {
    if (event.target.files.length) {
      if (this.serviceElectron.isElectron()){
        ...
        this.store.dispatch({ type: CURRENT_TREE, payload:{name:dir.name,path:dir.path}})
      }
        
    }
    this.inputSelectFolder.nativeElement.value=null
  }
      ...
      fs.readdirSync(dir.path)
        .forEach(element => {
          ...
          if (type === 'D') {
            ...
            if(this.localplaylist.filter(x=>x.path===path.join(dir.path, element)).length===0)
              this.fileList.push({
                name: element,
                path: path.join(dir.path, element),
                type: type,
  ...
  ngOnInit() {
    this.store.select("LOCAL").subscribe((state) => {
      if (state.tree)
        this.readDir(state.tree)
    })
  }
  toggleFileList() {
    this.inputFilterFolder.nativeElement.value=""
    this.inputFilterFile.nativeElement.value=""
    this.folderselection.nativeElement.addEventListener("animationend",
      () => {
    
        this.folderselection.nativeElement.classList.add("d-none");
        this.playlistselection.nativeElement.classList.remove('d-none', 'animated', 'fadeOutLeft')
        this.playlistselection.nativeElement.classList.add('animated', 'fadeInLeft')

      },{once:true,capture:true,passive:true})
    this.folderselection.nativeElement.classList.remove('animated', 'fadeInLeft')
  ...
  toggleFolderList() {
    this.inputFilterFolder.nativeElement.value=""
    this.inputFilterFile.nativeElement.value=""
    this.playlistselection.nativeElement.addEventListener("animationend",
      () => {
        this.playlistselection.nativeElement.classList.add("d-none");
        this.folderselection.nativeElement.classList.remove('d-none', 'animated', 'fadeOutLeft')
        this.folderselection.nativeElement.classList.add('animated', 'fadeInLeft');
      },{once:true,capture:true,passive:true})
    this.playlistselection.nativeElement.classList.remove('animated', 'fadeInLeft')
    this.playlistselection.nativeElement.classList.add('animated', 'fadeOutLeft')
  }
  toggleMenu(){
