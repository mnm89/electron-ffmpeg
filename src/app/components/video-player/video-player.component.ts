...
export class VideoPlayerComponent implements OnInit, OnDestroy {
  ...
  onCommandeData(sourceBuffer:SourceBuffer, chunk) {
  ...
  prepareAudioMediaSource(path){
    var mediaSource = new MediaSource();
    this.audioplayer.nativeElement.src = URL.createObjectURL(mediaSource);
    mediaSource.addEventListener('sourceopen',this.onAudioMediaSourceOpen.bind(this,mediaSource,path))
    
  }

  async prepareMediaSource(path) {
   
    this.ffprobData = await this.electronService.ffprobe(path)
    if (this.hasAudiotrack())
     this.prepareAudioMediaSource(path);
    ...
    mediaSource.addEventListener('sourceopen', this.onMediaSourceOpen.bind(this, mediaSource, path))
  }
...
hasAudiotrack():boolean{
  if(this.ffprobData)
return this.ffprobData.streams.filter(x => x.codec_type === "audio").length > 0
return false
}
hasNext(){
